...
Page({
  ...
  onLoad() {
    ...
    varName = setInterval(() => {  this.drawCircle();},500);
    wx.getNetworkType({
      success(res) {
        // 返回网络类型, 有效值：
        // wifi/2g/3g/4g/unknown(Android下不常见的网络类型)/none(无网络)
        that.netWorkTypeFun(res.networkType);
      }
    })
    wx.getSystemInfo({
      success(res) {
    ...
    this.getList('https://enear.babybus.com/api/player/?platform=1&ctv=1.0.0');
  ...
  listenError(info) {
    ...
    let errorInfo = '';
    switch (info.errCode) {
      case 10001: errorInfo = '系统错误';
        break;
      case 10002: errorInfo = '网络错误';
        break;
      case 10003: errorInfo = '文件错误';
        break;
      case 10004: errorInfo = '格式错误';
        break;
  ...
  drawCircle(defualt = false) {
    const { size, firstPlay } = this.data
    var drawArc = (s, e,eb) => {
    ...
    var animation = () => {
      ...
      if (backgroundAudioManager.paused&&firstPlay){
        drawStep = 0.1;
      };
   ...
   */
  rotateAni(n) {
    this.animation.rotate(14400 * (n)).step();
  ...
  play() {
    ...
    console.log(NowSong)
    this.setBackgroundInfo(NowSong.en_name, NowSong.cover);
